...
import {
  ...
  Users, Lock, ArrowLeft, Settings, Sparkles,
} from "lucide-react";
    ...
    <>
      ...
      <div className="page-scroll-content flex-1 overflow-y-auto custom-scrollbar">
        <div className="max-w-4xl mx-auto px-8 py-8">
          ...
          {/* Batch info hero */}
          <div className="relative overflow-hidden rounded-xl border border-hairline p-6 mb-8" style={{ backgroundColor: "rgb(10,13,18)" }}>
            <div className="pointer-events-none absolute -right-8 -top-16 w-40 h-40 rounded-full opacity-15 blur-3xl"
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {canManageChannels && (
              <button onClick={() => setCreating((v) => !v)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border-none text-black"
          ...
          {creating && (
            <div className="rounded-xl border border-accent-200 p-3 mb-3 flex items-center gap-2 bg-accent-50">
              ...
              <button onClick={() => newName.trim() && createChannel.mutate(newName.trim())} disabled={!newName.trim() || createChannel.isPending}
                className="px-3 py-1 rounded-md text-xs font-bold disabled:opacity-40 text-black border-none"
                style={{ background: "linear-gradient(rgb(59,130,255) 17%,rgb(0,219,232) 100%)" }}>
                Create
              </button>
            ...
            {channels?.map((ch: any) => {
              ...
              return (
                <div key={ch.id}
                  ...
                  <div className="w-8 h-8 rounded-lg border border-hairline flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "rgb(5,7,10)" }}>
