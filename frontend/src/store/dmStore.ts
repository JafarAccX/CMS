...
export const useDmStore = create<DmState>()((set) => ({
  ...
  addOptimisticMessage: (convId, msg) =>
    set((state) => ({
      messages: {
        ...state.messages,
        [convId]: [...(state.messages[convId] || []), { ...msg, isOptimistic: true }],
      },
    })),
  ...
  removeOptimisticMessage: (convId, tempId) =>
    set((state) => ({
      messages: {
        ...state.messages,
        [convId]: (state.messages[convId] || []).filter((m) => m.tempId !== tempId),
      },
    })),
  ...
  updateConversationPreview: (convId, msg) =>
    set((state) => ({
      conversations: state.conversations.map((c) =>
        c.id === convId ? { ...c, lastMessage: { id: msg.id, content: msg.content, sender_id: msg.sender_id, is_read: msg.is_read, created_at: msg.created_at }, updated_at: msg.created_at } : c
      ).sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()),
    })),

  // Mark a conversation as read in the sidebar preview
  ...
  markMessagesRead: (convId) =>
    set((state) => ({
      messages: {
        ...state.messages,
        [convId]: (state.messages[convId] || []).map((m) => ({ ...m, is_read: true })),
      },
    })),
}));
