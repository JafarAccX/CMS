...
import {
  ...
  LogOut, ChevronDown, ChevronRight, ChevronLeft, Folder, BookOpen,
  Pin, Plus, UserPlus,
} from "lucide-react";
...
function NavItem({
  ...
  return (
    <Link to={getUrl(href)} className="block">
      <div
...
export default function AppShell({ children }: AppShellProps) {
  ...
  const logout = useCmsAuthStore((s) => s.logout);
  const batches = useCmsBatchStore((s) => s.batches);
  const isEmbed = useCmsUiStore((s) => s.isEmbed);
  const isSidebarCollapsed = useCmsUiStore((s) => s.isSidebarCollapsed);
  const toggleSidebar = useCmsUiStore((s) => s.toggleSidebar);
  ...
  return (
    <div className="w-full h-full flex bg-surface overflow-hidden relative" style={{ background: "var(--ax-bg)" }}>
      {/* Decorative orbs */}
      ...
      {/* ── Expand Sidebar Float Button ── */}
      {isSidebarCollapsed && !hideGlobalSidebar && (
        <button
          onClick={toggleSidebar}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-50 p-2 rounded-r-lg border border-l-0 border-hairline flex items-center justify-center cursor-pointer shadow-lg hover:translate-x-0.5 active:translate-x-0 transition-all"
          style={{
            background: "var(--ax-panel-2)",
            borderColor: "var(--ax-border)",
            borderLeft: "none",
            boxShadow: "0 4px 20px -6px rgba(0,0,0,0.5), 0 0 10px rgba(59,130,255,0.15)",
          }}
          title="Expand sidebar"
        >
          <ChevronRight className="w-4 h-4" style={{ color: "var(--ax-sidebar-muted)" }} />
        </button>
      )}
      ...
      <aside className="app-sidebar w-64 flex-shrink-0 hidden lg:flex flex-col border-r border-hairline relative z-10 py-6 overflow-hidden"
        style={{
          width: isSidebarCollapsed ? "0px" : "var(--ax-sidebar-w)",
          paddingLeft: isSidebarCollapsed ? "0px" : undefined,
          paddingRight: isSidebarCollapsed ? "0px" : undefined,
          borderRightWidth: isSidebarCollapsed ? "0px" : undefined,
          background: "var(--ax-sidebar-bg)",
          boxShadow: isSidebarCollapsed ? "none" : "1px 0 8px var(--ax-border), 12px 0 34px -30px rgba(30,64,175,0.45)",
          transition: "width 0.2s cubic-bezier(0.4, 0, 0.2, 1), padding 0.2s ease, border-width 0.2s ease, box-shadow 0.2s ease",
        }}
      >
        ...
        {!isEmbed && (
        <Link to={getUrl("/")} className="block px-6 pb-5 border-b border-hairline hover:bg-[var(--ax-hover)] transition-colors">
          <div
        ...
        {/* Sidebar Header with Collapse Button (only visible when not collapsed) */}
        {!isSidebarCollapsed && (
        <div className="px-6 py-2.5 flex items-center justify-between border-b border-hairline mb-2 select-none shrink-0" style={{ borderColor: "var(--ax-border)" }}>
          <span className="text-[10px] font-bold tracking-[0.08em] uppercase" style={{ color: "var(--ax-sidebar-section)" }}>Platform Nav</span>
          <button
            onClick={toggleSidebar}
            className="p-1 hover:bg-[var(--ax-hover)] rounded-md transition-colors flex items-center justify-center cursor-pointer text-dim hover:text-primary"
            style={{ color: "var(--ax-sidebar-muted)" }}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-1">
          ...
          <SidebarSection label="COURSE GROUPS">
            {activeBatches.slice(0, 3).map((b) => (
              <Link key={b.id} to={getUrl(`/batch/${b.id}`)} className="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-[var(--ax-hover)] transition-colors cursor-pointer">
          ...
          {isMentor && (
            <Link to={getUrl("/mentor")} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-[var(--ax-hover)] transition-colors">
          ...
          {isLearner && (
            <Link to={getUrl("/dm?askMentor=1")} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-[var(--ax-hover)] transition-colors">
              <BookOpen className="w-[15px] h-[15px]" style={{ color: "var(--ax-sidebar-muted)" }} />
        ...
        <div className="px-4 mt-2">
          ...
          {isAdmin && (
            <Link to={getUrl("/admin")} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-[var(--ax-hover)] transition-colors mb-1">
