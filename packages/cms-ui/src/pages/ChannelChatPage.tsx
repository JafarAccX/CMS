...
export default function ChannelChatPage() {
  ...
  const messages = useCmsMessageStore((s) => s.messages);
  const setMessages = useCmsMessageStore((s) => s.setMessages);
  const rightPanelOpen = useCmsUiStore((s) => s.rightPanelOpen);
  const toggleRightPanel = useCmsUiStore((s) => s.toggleRightPanel);
  const typingUsers = useCmsUiStore((s) => s.typingUsers);
  const onlineUsers = useCmsUiStore((s) => s.onlineUsers);
  const { joinChannel, leaveChannel, sendMessage, startTyping, stopTyping, toggleReaction } = useCmsSocket();
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      useCmsMessageStore.getState().addOptimisticMessage(channelId, {
        id: tempId,
        tempId,
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          if (targetChannelId && user) {
            useCmsMessageStore.getState().addOptimisticMessage(targetChannelId, {
              id: tempId,
              tempId,
          ...
          if (targetChannelId) useCmsMessageStore.getState().removeOptimisticMessage(targetChannelId, tempId);
    ...
    <div className="flex h-full min-h-0 bg-surface text-primary relative overflow-hidden" style={{ background: "var(--ax-bg)" }}>
      ...
      <aside className={`${isMobileSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:relative inset-y-0 left-0 z-40 w-60 figma-panel border-y-0 border-l-0 flex min-h-0 flex-col shrink-0 transition-transform duration-300 ease-in-out lg:flex lg:h-full`} style={{ borderRadius: 0 }}>
        <div className="p-3.5 border-b border-hairline space-y-2">
          <Link to={getUrl(`/batch/${batchId}`)} className="flex items-center gap-2 text-dim hover:text-primary text-sm transition-colors">
            <ArrowLeft className="w-4 h-4" />
            {batch?.name || "Batch"}
          </Link>
          <Link to={getUrl("/dm")} className="flex items-center gap-2 text-accent-400 hover:text-accent-300 text-sm relative transition-colors">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-0.5">
          ...
          {channels?.map((c: any) => (
            <Link
              key={c.id}
              to={getUrl(`/batch/${batchId}/channel/${c.id}`)}
          ...
          {isGuest && (
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-[10px] p-3 mb-4 text-center">
              <p className="text-amber-300 text-sm">
                Guest mode. <Link to={getUrl("/register")} className="underline">Sign up</Link> for full access.
              </p>
            </div>
          )}
          {noAccess && !isGuest && (
